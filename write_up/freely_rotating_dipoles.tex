...
With the magnetic field for a dipole at some point of interest $\boldsymbol r_i$ being given by
...
\begin{figure}[h]
	\includegraphics[width=0.4\linewidth ]{./images/orbital_animatic.png} 
	\includegraphics[width=0.5\linewidth ]{./images/spinning_animatic.png} 

%    \subfloat[label 1]{{\includegraphics[width=5cm]{./images/orbital_animatic.png} }}%
%    \subfloat[label 1]{{\includegraphics[width=5cm]{./images/spinning_animatic.png} }}%
...
\begin{equation}
	\ddot \phi_t = -\frac{20}{4}\sin(\phi_t +2/5\phi_t) =  -5\sin\left ( \frac{7}{5}\phi_t \right )\approx -7\phi_t
\end{equation}
...
An adaptive Runge-Kutta 4 method was implemented to calculate the trajectory of the system using two different sized time steps, both made smaller until they agreed within a specified precision. To determine if a trajectory has returned through it's original point in phase space $\boldsymbol{\Gamma}(0)=\boldsymbol{\Gamma}_0$ linear interpolation was used between steps $\boldsymbol{\Gamma}_n$ and $\boldsymbol{\Gamma}_{n+1}$ to get an estimated time for when the initial value was revisited. This generated 6 estimated recurrence times. If all 6 calculated values fell between $t_n$ and $t_{n+1}$ the average was taken and stored. This process was continued until 75 time units had elapsed and then a fit was made to the equation $t_m = t_0*m$, the slope $t_0$ was taken to be the period.
...
\bibliography{./double_magnet_bib.bib}
