...
This kinetic energy is the same expression in Taylor's \cite{taylor2005classical} for center of mass coordinates with the extra addition of rotational kinetic energy accounting for the spinning the spheres can undergo. 
The potential energy for dipole 1 is merely the dot product of its dipole moment with the local field, which in this case is caused entirely by dipole 2.
\begin{equation}
...
It might be noted that this Lagrangian takes into account only magneto-static interactions which ignores the resulting damping one would see from accelerating dipoles which would necessarily happen anytime the system is away from equilibrium. The scale of those corrections will be addressed in their section later.
...
\end{equation}
% rewrite to emphasise special nature permitting this chain of reasoning and highlight when it would break down
...
\begin{equation}
\tau_{decay}=180 c^3 (L_0^2 \omega_0^2\frac{L_0}{T_0})^{-1}T_0. 
\end{equation}
...
%spend a bit more time explaining why it's unstable
...
The rates of change for the momenta clearly denote a conserved quantity we'll call total angular moment and denote as $L_t = p_\theta + 2p_{\phi_t}$. With this we can produce the expression
...
\begin{equation}
  \begin{multlined}
	...
	= \phi_{t0} + \int_0^t \kern-.33em 10 (L_t-p_{ \theta }) dt
	\\
	= \phi_{t0} + 10L_tt -10 \int_0^t \kern-.33em p_{ \theta } dt
  \end{multlined}
\end{equation}
